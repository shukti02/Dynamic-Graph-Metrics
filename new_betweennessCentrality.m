function [ betweennessCent, durationShortestPaths, nFastestPaths ] = ...
    new_betweennessCentrality(my_network,directed)
    ...
    for p = 1:length(startingNode)
        ...
        if dur == 2
            ...
            for n = 1:length(node2)
                yn = my_setOfInfluence(node2(n),my_network,directed,t,t+dur-1,[],nNodes);
            ...
            for t_i = t+1:t+dur-2
                ...
                for n1 = 1:size(paths,1)
                    ...
                    for n = 1:length(nodet_i)
                        yn = my_setOfInfluence(nodet_i(n),my_network,directed,t_i+1,...
                                t+dur-1,[],nNodes);
                        if ismember(endingNode(p),yn)
                            keeps = [keeps; n];
                        end
